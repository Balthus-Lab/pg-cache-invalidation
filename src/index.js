        ...
        .begin((sql) =>
          [
            ...
            BEGIN
              FOR obj IN
              SELECT
                format('SELECT pg_notify(''cacheinvalid__notify'', jsonb_build_object(''op'', %L, ''table'', %L, ''purge'', %L::jsonb, ''key_fields'', jsonb_agg(jsonb_build_object(%s)))::varchar(7999)) FROM %s HAVING count(sq.*) > 0', op, o_table_name, array_to_json(ARRAY (
                      SELECT
                        sq.name FROM (
                          SELECT
                            o_table_name AS name
                          UNION
                          SELECT
                            u.view_name AS name FROM information_schema.view_table_usage u
                          WHERE
                            u.table_schema NOT IN ('information_schema', 'pg_catalog')
                          AND u.table_name = o_table_name) sq
                    WHERE (
                      SELECT
                        jsonb_path_query_array(metadata::jsonb, '$.sources[*].tables[*].table.name')
                      FROM "hdb_catalog".hdb_metadata) ? sq.name)), string_agg(format('''%1$s'', %1$s', kcu.column_name), ', '), CASE WHEN need_join THEN
              ...
              WHERE
                ...
                AND tc.constraint_type = 'PRIMARY KEY' LOOP
                  EXECUTE obj.x;
                END LOOP;
              RETURN NEW;
            END;
